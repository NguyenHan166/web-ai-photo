...
export async function POST(
    ...
    try {
        ...
        const feature =
            featureParam || request.headers.get("x-feature-type") || "upscale";
        ...
        switch (feature) {
            ...
            case "replace-bg":
                ...
                break;
            case "style":
                // Use the replace-style route for style transfers
                endpoint = "/style";
                break;
            case "comic":
        ...
        if (isJson) {
            ...
            // If replace-bg expects fg/bg, ensure names are correct
            if (feature === "replace-bg") {
                const fg = formData.get("fg");
                const bg = formData.get("bg");
                // The original form may use "image"/"background"; move them if present
                if (!fg && formData.get("image")) {
                    const img = formData.get("image") as File;
                    if (img) formData.set("fg", img as any);
                }
                if (!bg && formData.get("background")) {
                    const b = formData.get("background") as File;
                    if (b) formData.set("bg", b as any);
                }
            }
